  ...
  try {
    const rows = await db.prepare(`
      SELECT s.id as consultant_id, s.name, s.department, s.mobile,
             CAST(COUNT(DISTINCT cp.id) AS INTEGER)  as payment_count,
             COALESCE(SUM(cp.amount), 0)             as total_paid,
             CAST(COUNT(DISTINCT a.id) AS INTEGER)   as appointment_count
      FROM staff s
      ...
      GROUP BY s.id, s.name, s.department, s.mobile
      ORDER BY total_paid DESC
    `).all();
    const grand_total = rows.reduce((sum, r) => sum + parseFloat(r.total_paid || 0), 0);
...
router.get('/:id/appointments', async (req, res) => {
  try {
    const appointments = await db.prepare(`
      SELECT a.*, p.first_name, p.last_name, p.mobile as patient_mobile
      FROM appointments a
      LEFT JOIN patients p ON a.patient_id = p.id
...
router.post('/:id/payments', async (req, res) => {
  try {
    ...
    const payment = await db.prepare('SELECT * FROM consultant_payments WHERE id = ?').get(result.lastInsertRowid);
...
router.delete('/payments/:paymentId', async (req, res) => {
  try {
    await db.prepare('DELETE FROM consultant_payments WHERE id = ?').run(req.params.paymentId);
