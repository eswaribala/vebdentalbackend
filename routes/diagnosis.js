...
router.get('/:id', async (req, res) => {
  try {
    ...
    const treatmentPlans = await db.prepare('SELECT * FROM treatment_plans WHERE diagnosis_id = ?').all(req.params.id);
    const prescriptions  = await db.prepare('SELECT * FROM prescriptions  WHERE diagnosis_id = ?').all(req.params.id);
...
router.post('/', async (req, res) => {
  try {
    ...
    const diag = await db.prepare('SELECT * FROM diagnosis WHERE id = ?').get(result.lastInsertRowid);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    await db.prepare('UPDATE diagnosis SET chief_complaint=?, clinical_notes=?, tooth_chart=? WHERE id=?').run(
      chief_complaint, clinical_notes,
      typeof tooth_chart === 'object' ? JSON.stringify(tooth_chart) : tooth_chart,
      req.params.id
    );
    const diag = await db.prepare('SELECT * FROM diagnosis WHERE id = ?').get(req.params.id);
...
router.post('/:id/treatments', async (req, res) => {
  try {
    ...
    const plan = await db.prepare('SELECT * FROM treatment_plans WHERE id = ?').get(result.lastInsertRowid);
...
router.get('/:id/treatments', async (req, res) => {
  try {
    const plans = await db.prepare('SELECT * FROM treatment_plans WHERE diagnosis_id = ? ORDER BY created_at').all(req.params.id);
    res.json({ success: true, data: plans });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.put('/treatments/:treatId', async (req, res) => {
  try {
    const { tooth_number, treatment_type, description, cost, status } = req.body;
    await db.prepare('UPDATE treatment_plans SET tooth_number=?, treatment_type=?, description=?, cost=?, status=? WHERE id=?')
      .run(tooth_number, treatment_type, description, cost, status, req.params.treatId);
    const plan = await db.prepare('SELECT * FROM treatment_plans WHERE id = ?').get(req.params.treatId);
    res.json({ success: true, data: plan });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.delete('/treatments/:treatId', async (req, res) => {
  try {
    await db.prepare('DELETE FROM treatment_plans WHERE id = ?').run(req.params.treatId);
...
router.post('/:id/prescriptions', async (req, res) => {
  try {
    ...
    const rx = await db.prepare('SELECT * FROM prescriptions WHERE id = ?').get(result.lastInsertRowid);
...
router.get('/:id/prescriptions', async (req, res) => {
  try {
    const rxList = await db.prepare('SELECT * FROM prescriptions WHERE diagnosis_id = ? ORDER BY rx_date DESC').all(req.params.id);
    res.json({ success: true, data: rxList });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

router.get('/patient/:patientId/treatments', async (req, res) => {
  try {
    const plans = await db.prepare('SELECT * FROM treatment_plans WHERE patient_id = ? ORDER BY created_at DESC').all(req.params.patientId);
    res.json({ success: true, data: plans });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
